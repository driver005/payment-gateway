...
func setLevel(l *logrus.Logger, o *options) {
	if o.level != nil {
		...
		var err error
		l.Level, err = logrus.ParseLevel(helper.Coalesce(
...
func setFormatter(l *logrus.Logger, o *options) {
	if o.formatter != nil {
		...
		format := helper.SwitchExact(helper.Coalesce(o.format, o.c.String("log.format"), os.Getenv("LOG_FORMAT")))
		switch {
...
func New(name string, version string, opts ...Option) *Logger {
	...
	return &Logger{
		...
		redactionText: helper.DefaultIfEmpty(o.redactionText, `Value is sensitive and has been redacted. To see the value set config key "log.leak_sensitive_values = true" or environment variable "LOG_LEAK_SENSITIVE_VALUES=true".`),
...
func (l *Logger) UseConfig(c configurator) {
	...
	l.redactionText = helper.DefaultIfEmpty(c.String("log.redaction_text"), l.redactionText)
