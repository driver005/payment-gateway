...
func (a *Admin) ListPriceList(context *fiber.Ctx) error {
...
func (a *Admin) CreatePriceList(context *fiber.Ctx) error {
	...
	r, err := a.r.ClientManager().CreatePriceList(context.Context(), &m)
...
func (a *Admin) UpdatePriceList(context *fiber.Ctx) error {
	var m models.PriceList
	...
	r, err := a.r.ClientManager().UpdatePriceList(context.Context(), &m)
	if err != nil {
		return context.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"message": err.Error(),
			"type":    "database_error",
...
func (a *Admin) DeletePriceList(context *fiber.Ctx) error {
	...
	r := a.r.ClientManager().DeletePriceList(context.Context(), Id)
	if r != nil {
		return context.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"message": err.Error(),
			"type":    "database_error",
