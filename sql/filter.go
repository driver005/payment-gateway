...
func orderBy(db *database.DB, params queryParams) *database.DB {
...
func paginate(db *database.DB, params queryParams) *database.DB {
	if params.All {
		return db
	}
...
func getColumnNameForField(field reflect.StructField) string {
	...
	databaseTag := field.Tag.Get("database")
	res := columnNameRegexp.FindStringSubmatch(databaseTag)
	if len(res) == 2 {
		return res[1]
	}
...
func expressionByField(
	db *database.DB, phrase string, modelType reflect.Type,
	operator func(reflect.StructField, string) clause.Expression,
...
//	type User struct {
//		database.Model
//		Username string `database:"uniqueIndex" filter:"param:login;searchable;filterable"`
//		// `param` defines custom column name for the query param
//		FullName string `filter:"searchable"`
//	}
func FilterByQuery(context *fiber.Ctx, config int) func(db *database.DB) *database.DB {
