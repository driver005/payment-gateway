...
func (h *Handler) SetRoutes(r fiber.Router) {
	r.Get("/credit_notes", h.RouteList)
	r.Post("/credit_notes", h.RouteCreate)
	r.Get("/credit_notes/:id", h.RouteGet)
	r.Post("/credit_notes/:id", h.RouteUpdate)
	r.Get("/credit_notes/:id/lines", h.RouteLines)
...
func (h *Handler) RouteVoid(context *fiber.Ctx) error {
	...
	m.Id = Id
	m.VoidedAt = time.Now().UTC().Round(time.Second)
